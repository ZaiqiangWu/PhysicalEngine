...
public:
    ...
    void UpdateMove()
    {
        GLfloat velocity = this->MovementSpeed * 0.02;
        if (isForward)
...
void curse_poscallback(GLFWwindow* window, double x, double y)
{
    ...
    anglex += 360 * deltax / (GLfloat)WinW;//根据屏幕上鼠标滑动的距离来设置旋转的角度  
    angley += 360 * deltay / (GLfloat)WinH;
    anglez += 360 * deltay / (GLfloat)WinH;
    oldx = WinW / 2;//x;//记录此时的鼠标坐标，更新鼠标坐标  
    oldy = WinH / 2;// y;//若是没有这两句语句，滑动是旋转会变得不可控  
    //cout << "x:" << x << endl;
    //cout << "y:" << y << endl;
